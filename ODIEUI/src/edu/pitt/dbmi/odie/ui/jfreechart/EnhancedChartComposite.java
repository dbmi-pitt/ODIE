 ...
 *
 * -------------------
 * ODIEChartComposite.java
 * -------------------
 * (C) Copyright 2008-2010, by Girish Chavan
 *
 ...
 *                   
 *                   
 * Adapted for ODIE By: Girish Chavan
 * This file was originally called ChartComposite.java and was distributed with JFreeChart v1.0.10
 * It has been modified for use in the ODIE (Ontology Development & Information Extraction) project.
 * Modifications work only for bar charts. It will definitely not work for pie charts without 
 * further updates.
 * 
 * Changes
 * 
 * - Modified zoom rectangle to only work when left mouse button + ctrl key is down.
 * - Added selection rectangle for regular mouse drag with left button.
 * - Selection rectangle resized and moves when chart is resized. It does this only upto
 *   scale factor of 1 and above. The scaleX var. never seems to go below 1. This is a problem from
 *   the original ChartComposite.
 * - getScreenDataArea uses Math.round for a more accurate location.
 * - Modified Methods:
 * 		mouseUp,mouseMove,mouseDown,paintControl
 * - Added Methods:
 * 		getEnclosedEntities(Rectangle selectionRectangle),
 * 		translateJava2DToScreen(Rectangle rectangle),
 * 		getEntityForLocation(double x, double y)
 * 		getSelectedEntities();
 */
 ...
 * 
 */
public class EnhancedChartComposite extends Composite implements
	...
	// a hack to make the selection rectangle perfectly drawn within the axis.
	private static final int FIX = 2;

	private Color selectionBackground = new Color(Display.getDefault(),
			new RGB(0, 0, 0));
	private Color selectionForeground = new Color(Display.getDefault(),
			new RGB(190, 225, 240));
	...
	private org.eclipse.swt.graphics.Point selectionPoint = null;

	/** The zoom rectangle (selected by the user with the mouse + CTRL key). */
	private transient Rectangle zoomRectangle = null;

	/** The selection rectangle (selected by the user with the mouse). */
	private transient Rectangle selectionRectangle = null;
	...
	List<ChartEntity> selectedEntities = new ArrayList<ChartEntity>();
	 ...
	 */
	public EnhancedChartComposite(Composite comp, int style) {
	 ...
	 *            the chart.
	 */
	public EnhancedChartComposite(Composite comp, int style, JFreeChart chart) {
	...
	public EnhancedChartComposite(Composite comp, int style, JFreeChart chart,
		...
		this(comp, style, chart, width, height, width,
				DEFAULT_MINIMUM_DRAW_HEIGHT, width, height,
				// DEFAULT_WIDTH,
				// DEFAULT_HEIGHT,
	...
	public Rectangle getScreenDataArea() {
		...
		int x = (int) Math
				.round((dataArea.getX() * this.scaleX + clientArea.x));
		int y = (int) Math
				.round((dataArea.getY() * this.scaleY + clientArea.y));
			...
			if (entities != null) {
				...
				if (entity != null) {
					...
					if (entity instanceof CategoryItemEntity) {
						Datapoint dp = (Datapoint) ((CategoryItemEntity) entity)
								.getColumnKey();
						result = (dp.getConceptId() == null ? dp.getType() : dp
								.getConceptId());
					}
					// else{
		...
		if (pf2 != pf) {
			...
			if (job.printDialog()) {
				try {
					...
					MessageBox messageBox = new MessageBox(this.canvas
	...
	public void mouseDown(MouseEvent event) {
		...
		// new entity code
		ChartEntity entity = getEntityForLocation(javaPoint.getX(), javaPoint
				.getY());
		if (entity != null)
		...
		if (event.button == 1) {
			...
			if ((event.stateMask & SWT.CONTROL) != 0) {
				...
				this.selectionPoint = getPointInRectangle(event.x, event.y,
						scaledDataArea);
			}
		}
	...
	public void clearSelection(boolean redraw, boolean fireSelectionChange) {
	...
	private ChartEntity getEntityForLocation(double x, double y) {
		if (this.info != null) {
			...
			if (entities != null) {
				...
				Point2D javaPoint = translateScreenToJava2D(new Point(rect.x,
						rect.y));
				if (this.orientation == PlotOrientation.HORIZONTAL) {
					...
					while (entity == null
							&& y < (javaPoint.getY() + (rect.height * scaleY))) {
						entity = entities.getEntity(x, y);
	...
	private void boundSelectionRectangle() {
		...
		if (selectedEntities.size() > 0) {
			...
			ChartEntity last = selectedEntities
			...
			Point2D screenPoint = translateScreenToJava2D(new Point(
					screenArea.x, screenArea.y));

			if (this.orientation == PlotOrientation.VERTICAL) {
				selectionRectangle = new Rectangle(firstp.x-FIRSTENTITYERROR, (int) screenPoint
						.getY(), lastp.x + last.getArea().getBounds().width
						- firstp.x, (int) (screenArea.height / scaleY));
			} else {
				selectionRectangle = new Rectangle((int) screenPoint.getX(),
						firstp.y, (int) (screenArea.width / scaleX), lastp.y
								+ last.getArea().getBounds().height - firstp.y);
			}
		}
	}
	...
	private List<ChartEntity> getEnclosedEntities(Rectangle selectionRectangle) {
		...
		if (orientation == PlotOrientation.VERTICAL) {
			...
			int y = selectionRectangle.y + selectionRectangle.height
					- DISTANCE_FROM_AXIS;
	...
	public void mouseMove(MouseEvent event) {
		...
		if (this.zoomPoint != null) {
			...
			Rectangle scaledDataArea = getScreenDataArea(this.selectionPoint.x,
					this.selectionPoint.y);

			org.eclipse.swt.graphics.Point movingPoint = getPointInRectangle(
					event.x, event.y, scaledDataArea);

			if (this.orientation == PlotOrientation.VERTICAL) {
				Point2D selectionPointJava = translateScreenToJava2D(new Point(
						this.selectionPoint.x, scaledDataArea.y));
				this.selectionRectangle = new Rectangle(
						(int) selectionPointJava.getX(),
						(int) selectionPointJava.getY(),
						(int) ((movingPoint.x - this.selectionPoint.x) / scaleX),
						(int) (scaledDataArea.height / scaleY));
			} else {
				int ymax = Math.max(movingPoint.y, scaledDataArea.y);

				Point2D selectionPointJava = translateScreenToJava2D(new Point(
						scaledDataArea.x, this.selectionPoint.y));
				this.selectionRectangle = new Rectangle(
						(int) selectionPointJava.getX(),
						(int) selectionPointJava.getY(),
						(int) (scaledDataArea.width / scaleX),
	...
	public void paintControl(PaintEvent e) {
		...
		// Rectangle available = new
		// Rectangle(0,0,maximumDrawWidth,maximumDrawHeight);
		...
		// scaleY);
		// logger.debug(minimumDrawWidth +"," + minimumDrawHeight + "," +
		...
		// Draw the zoom rectangle and selection rectangle
		if (this.zoomRectangle != null) {

			e.gc.drawRectangle(this.zoomRectangle);
		} else if (this.selectionRectangle != null) {
			boundSelectionRectangle();
			Rectangle rect = translateJava2DToScreen(selectionRectangle);
			e.gc.setBackground(selectionBackground);
			e.gc.setAlpha(75);
	...
	/**
	 * Translates selectionRectangle co-ordinates and dimensions to screen
	 * co-ordinates and dimensions.
	 * 
	 * @param selectionRectangle
	 * @return Rectangle that is ready to be painted
	 */
	private Rectangle translateJava2DToScreen(Rectangle rectangle) {
		Point p = translateJava2DToScreen(new Point(rectangle.x, rectangle.y));
		return new Rectangle(p.x + FIX, p.y + FIX,
				(int) (rectangle.width * scaleX),
				(int) (rectangle.height * scaleY));
	}
	...
	public ISelection getSelection() {
		return new StructuredSelection(selectedEntities);
	}
	...
	protected void fireSelectionChanged(final SelectionChangedEvent event) {
		// logger.debug("Firing selection changed from EnhancedComposite");

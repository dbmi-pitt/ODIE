...
public class DocumentPerformanceTreeTable extends TreeViewer {


	public DocumentPerformanceTreeTable(Composite parent) {
	...
	void createColumns(List<Analysis> analyses) {
		...
		tc.setText("Documents");
		tc.setResizable(true);
		tc.setWidth(200);
		
		for(Analysis a:analyses){
			tc = new TreeColumn(getTree(), SWT.LEFT);
			tc.setText(a.getName() + ":" + "Concepts Found");
			tc.setResizable(true);
			tc.setWidth(50);
		}
	}
...
//					suggestionsComparator));
	...
	protected void attachProviders() {
		setContentProvider(new DocumentPerformanceContentProvider());
